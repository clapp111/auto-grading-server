...
def run_problem_ocr(self, job_id: int):
    """문제 영역을 잘라 OCR하여 문제 텍스트를 저장한다.

    문제지 PDF에서 지정된 영역을 잘라 Clova OCR로 인식하고 문제 본문에 저장한다. 외부 OCR
    호출 실패는 지수 백오프로 재시도하며, 그 외 오류는 FAILED로 기록한다.

    Args:
        job_id: 처리할 문제 OCR 잡 ID
    ...
    try:
        ...
        image_bytes = crop_region(
            pdf_bytes, region.page, region.x, region.y, region.w, region.h
        )
...
def run_model_answer_ocr(self, job_id: int):
    """모범답안 영역을 OCR하여 모범답안 텍스트를 저장한다.

    모범답안 PDF에서 지정된 영역을 잘라 문제 유형에 맞는 OCR 클라이언트로 인식하고
    저장한다. 외부 OCR 호출 실패는 지수 백오프로 재시도하며, 그 외 오류는 FAILED로 기록한다.

    Args:
        job_id: 처리할 모범답안 OCR 잡 ID
...
def run_student_id_ocr(self, job_id: int):
    """답안지의 이름·학번 영역을 OCR해 학생과 매칭한다.

    미매칭 답안지의 이름·학번을 인식해 학생을 찾거나 새로 만들고 답안지에 연결한 뒤 상태를
    MATCHED로 바꾼다. 학번 인식 실패나 같은 학생 중복 매칭은 부분 실패로 기록한다. 외부 OCR
    호출 실패는 지수 백오프로 재시도하며, 그 외 오류는 FAILED로 기록한다.

    Args:
        job_id: 처리할 학생 식별 OCR 잡 ID
    ...
    try:
        ...
        job.celery_task_id = self.request.id
        job.progress_json = _build_progress(
            0, 0, "PREPARING", "학생 식별 OCR을 준비 중입니다."
        )
        db.commit()
        ...
        if failed_targets:
            job.status = JobStatus.FAILED
            job.progress_json = _build_progress(
                total, total, "FAILED", "일부 답안지의 학생 식별에 실패했습니다."
            )
            job.error_json = {
            ...
            job.status = JobStatus.DONE
            job.progress_json = _build_progress(
                total, total, "DONE", "학생 식별 OCR이 완료되었습니다."
            )
        db.commit()
...
def run_answer_ocr(self, job_id: int):
    """학생 답안 영역들을 OCR해 결과를 저장한다.

    서술형·손코딩 영역만 대상으로 하며, 같은 답안지 PDF를 캐시해 순차 인식한다. 이미 영역
    수정 시각보다 최신인 OCR 결과가 있으면 건너뛴다. 일부 영역 실패는 부분 실패로 기록하고,
    외부 OCR 호출 실패는 지수 백오프로 재시도하며, 그 외 오류는 FAILED로 기록한다.

    Args:
        job_id: 처리할 답안 OCR 잡 ID
    ...
    try:
        ...
        layout_mode = (
            LayoutMode(scope_layout_mode) if scope_layout_mode else exam.layout_mode
        )
        sheet_filter = [
            AnswerSheet.exam_id == exam.exam_id,
        ...
        problem_map = {
            p.problem_id: p
            for p in db.query(Problem).filter(Problem.problem_id.in_(problem_ids)).all()
        }
        ...
        for index, (sheet, region) in enumerate(targets, start=1):
            try:
                ...
                Exception
            ) as e:  # noqa: BLE001 - Record a failed region and continue the remaining OCR targets.
                failed += 1
                failed_targets.append(
                    {
            ...
            if index % max(1, total // 10) == 0 or index == total:
                job.progress_json = _build_progress(
                    index, total, "OCR", f"{index}/{total} 답안 영역을 인식 중입니다."
                )
                db.commit()
        ...
        if failed > 0:
            job.status = JobStatus.FAILED
            job.progress_json = _build_progress(
                total, total, "FAILED", "일부 답안 영역의 OCR에 실패했습니다."
            )
            job.error_json = {
                ...
                "category": "provider",
                "failedTargets": {
                    "answerRegionIds": [t["answerRegionId"] for t in failed_targets]
                },
            }
        else:
            job.status = JobStatus.DONE
            job.progress_json = _build_progress(
                total, total, "DONE", "답안 OCR이 완료되었습니다."
            )
