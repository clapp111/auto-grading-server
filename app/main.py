...
async def member_not_found_handler(request: Request, exc: MemberNotFoundError) -> JSONResponse:
    return JSONResponse(
        status_code=404,
        content={"data": None, "meta": None, "error": {"code": "MEMBER_NOT_FOUND", "message": "해당 이메일의 사용자를 찾을 수 없습니다."}},
    )


@app.exception_handler(InvitationNotFoundError)
async def invitation_not_found_handler(request: Request, exc: InvitationNotFoundError) -> JSONResponse:
    return JSONResponse(
        status_code=404,
        content={"data": None, "meta": None, "error": {"code": "INVITATION_NOT_FOUND", "message": "초대를 찾을 수 없습니다."}},
    )


@app.exception_handler(SelfInvitationError)
async def self_invitation_handler(request: Request, exc: SelfInvitationError) -> JSONResponse:
    return JSONResponse(
        status_code=400,
        content={"data": None, "meta": None, "error": {"code": "SELF_INVITATION", "message": "자기 자신은 초대할 수 없습니다."}},
    )


@app.exception_handler(AlreadyExamMemberError)
async def already_exam_member_handler(request: Request, exc: AlreadyExamMemberError) -> JSONResponse:
    return JSONResponse(
        status_code=409,
        content={"data": None, "meta": None, "error": {"code": "ALREADY_EXAM_MEMBER", "message": "이미 시험에 참여 중인 사용자입니다."}},
    )


@app.exception_handler(InvitationAlreadyRespondedError)
async def invitation_already_responded_handler(request: Request, exc: InvitationAlreadyRespondedError) -> JSONResponse:
    return JSONResponse(
        status_code=409,
        content={"data": None, "meta": None, "error": {"code": "INVITATION_ALREADY_RESPONDED", "message": "이미 처리된 초대입니다."}},
    )


@app.exception_handler(InvitationAlreadyPendingError)
async def invitation_already_pending_handler(request: Request, exc: InvitationAlreadyPendingError) -> JSONResponse:
    return JSONResponse(
        status_code=409,
        content={"data": None, "meta": None, "error": {"code": "INVITATION_ALREADY_PENDING", "message": "이미 대기 중인 초대가 있습니다."}},
    )


@app.exception_handler(ExamOwnerRequiredError)
async def exam_owner_required_handler(request: Request, exc: ExamOwnerRequiredError) -> JSONResponse:
    return JSONResponse(
        status_code=403,
        content={"data": None, "meta": None, "error": {"code": "EXAM_OWNER_REQUIRED", "message": "시험 소유자만 수행할 수 있습니다."}},
    )
