...
async def issue_model_answer_url(
    ...
    return ApiResponse(
        data=service.issue_model_answer_url(exam_id, current_member.member_id, request)
    )


@router.get(
    "/exams/{exam_id}/model-answers",
    response_model=ApiResponse[list[ModelAnswerResponse]],
)
async def list_model_answers(
    ...
    return ApiResponse(
        data=service.list_model_answers(exam_id, current_member.member_id)
    )
...
async def run_model_answer_ocr(
    ...
    return ApiResponse(
        data=service.run_ocr(problem_id, current_member.member_id, request)
    )
...
async def update_model_answer(
    ...
    return ApiResponse(
        data=service.update_model_answer(problem_id, current_member.member_id, request)
    )
