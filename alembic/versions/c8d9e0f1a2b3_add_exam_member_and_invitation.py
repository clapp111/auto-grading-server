...
def upgrade() -> None:
    op.create_table(
        "exam_member",
        sa.Column(
            "exam_member_id", sa.BigInteger(), autoincrement=True, nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["member_id"], ["member.member_id"], ondelete="CASCADE"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["exam_id"], ["exam.exam_id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["inviter_member_id"], ["member.member_id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["invitee_member_id"], ["member.member_id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("invitation_id"),
    )
    op.create_index("ix_invitation_exam_id", "invitation", ["exam_id"])
    op.create_index(
        "ix_invitation_invitee_member_id", "invitation", ["invitee_member_id"]
    )
    op.create_index(
        "ix_invitation_inviter_member_id", "invitation", ["inviter_member_id"]
    )
    # 대기 중인 초대는 (시험, 초대받는 사람)당 하나만 허용. 거절/취소 이력은 남기고 재초대를 허용하기 위해 부분 인덱스로 건다.
    op.create_index(
        "uq_invitation_pending",
        "invitation",
        ["exam_id", "invitee_member_id"],
        unique=True,
